...
Примеры из кода MavixWeb.

- **S — единственная ответственность.** Каждая страница кабинета — свой модуль
  (`public/js/pages/deliveries.js`, `operators.js`, `drones.js`, `cabinet.js`);
  Express-сервер делает только раздачу статики и прокси к API.
- **O — открытость/закрытость.** Переиспользуемая таблица `public/js/data-table.js`
  и кастомный дропдаун конфигурируются данными — новая таблица/список собирается
  настройкой, без правки логики компонента.
- **L — подстановка Лисков.** Страничные модули следуют единому контракту
  инициализации/загрузки данных — переключение разделов кабинета единообразно.
- **I — разделение интерфейсов.** Компоненты (таблица, дропдаун, формы) дают
  минимальный публичный API; страница использует только нужные.
- **D — инверсия зависимостей.** Базовый URL API не зашит, а инъектируется через
  `config.js` (из `API_BASE_URL`); фронтенд работает с любым адресом сервера.
- **DRY.** Общие `data-table.js`, `config.js` и компоненты вместо копипасты
  таблиц/форм по страницам.
- **KISS.** Ванильный JavaScript (модули-IIFE), без фреймворка и шага сборки —
  минимум зависимостей и слоёв.
- **YAGNI.** Нет фреймворка/бандлера и «универсальных» абстракций — реализованы
  только нужные страницы и операции кабинета.
